...
- **対応OS**: macOS (`aarch64-darwin` / `x86_64-darwin`), NixOS (`aarch64-linux`)
- **Host**: 複数マシンのホストネーム動的取得に対応
- **NixOS ターゲット**: Raspberry Pi 3B+ (macOS からのリモートデプロイ)
- **ユーザー**: `amemiya`
...
│   └── hardware/
│       └── nixos.nix      # Raspberry Pi 3B+ のハードウェア設定
...
### NixOS 固有設定 (`nixos/`)
- **システム設定** (`configuration.nix`): NetworkManager, OpenSSH, swapfile, パスワードなし sudo, タイムゾーン(Asia/Tokyo)
- **ハードウェア設定** (`hardware/nixos.nix`): Raspberry Pi 3B+ 向けのカーネルモジュール設定
  - **USB**: `dwc2` (BCM2837 内蔵 USB コントローラ)
  - **Ethernet**: `lan78xx` (LAN7515 USB 接続ギガビットイーサネット)
  - **カーネル**: mainline `linuxPackages` を使用（RPi fork はバイナリキャッシュになく、ビルドに 2.5h+ かかるため）
- **[nixos-hardware](https://github.com/NixOS/nixos-hardware)**: `raspberry-pi-3` モジュールを利用してブートローダーやハードウェア固有設定を自動化
...
**macOS から NixOS (Raspberry Pi) へのリモートデプロイ:**
ラズパイ等のメモリが少ないデバイス向けに、Mac側で重い評価処理を行ってからネットワーク越しにシステムを適用するコマンドを用意しています。

```bash
deploy-nixos <ユーザー名@ターゲットIP> <ホストネーム>
# 例: deploy-nixos amemiya@192.168.1.10 nixos
```

**初回セットアップ (Raspberry Pi):**
1. [NixOS aarch64 SD イメージ](https://nixos.org/download/)を SD カードに書き込む
2. ラズパイを起動し、初期ユーザー `nixos` でログイン
3. `passwd` でパスワードを設定、`ip addr` で IP アドレスを確認
4. Mac から `deploy-nixos nixos@<IP> nixos` で初回デプロイ
5. デプロイ後、`ssh amemiya@<IP>` でログインし `sudo passwd amemiya` でパスワードを設定
